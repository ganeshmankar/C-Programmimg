...
int main()
{
    ...
    while (i < 3)
    {
        printf("Employee %d:\nEnter the number of characters in your Employee Id\n", i+1);    //SINCE WE WANT TO START IT FROM 1
        scanf("%d", &chars);

        getchar();
        printf("Enter the value of a\n");    //COZ OF INPUT BUFFER HERE IF WE DONT USE GETCHAR() WE WONT BE ABLE TO GIVE VALUES FOR A AND B
        scanf("%c", &a);
        getchar();

        printf("Enter the value of b\n");
        scanf("%c", &b);
        ptr = (char *) malloc((chars+1)*sizeof(char));      //since string needs an extra null character
